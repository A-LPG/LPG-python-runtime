...
class DiagnoseParser(Stacks):
    ...
    def __init__(self, tokStream: TokenStream, prs: ParseTable, maxErrors: int = 0, maxTime: int = 0,
        ...
        self.tempStackTop: int = 0
        self.tempStack: list = []

        self.prevStackTop: int = 0
        self.prevStack: list = []

        self.nextStackTop: int = 0
        self.nextStack: list = []

        self.scopeStackTop: int = 0
    ...
    #
    def primaryPhase(self, error_token: int) -> RepairCandidate:
        #
    ...
    def mergeCandidate(self, state: int, buffer_position: int) -> int:
        name: str = self.tokStream.getName(self.buffer[buffer_position]) + self.tokStream.getName(
    ...
    #
    def getTermIndex(self, stck: list, stack_top: int, tok: int, buffer_position: int) -> int:
        #
    ...
    #
    def secondaryCheck(self, stack: list, stack_top: int, buffer_position: int, distance: int) -> bool:
        top: int = stack_top - 1
        while top >= 0:
    ...
    #
    def misplacementRecovery(self, repair: SecondaryRepairInfo, stack: list, stack_top: int, last_index: int,
                             stack_flag: bool):
    ...
    def secondaryRecovery(self, repair: SecondaryRepairInfo, stack: list, stack_top: int,
        ...
        while top >= 0 and repair.numDeletions >= stack_deletions:
            ...
            while (i <= (last_index - self.MIN_DISTANCE + 1) and
                ...
                ll: int = self.nasi(stack[top])
                while ll >= 0 and self.nasr(ll) != 0:
                    symbol: int = self.nasr(ll) + self.NT_OFFSET
