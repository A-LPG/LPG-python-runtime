...
class PrimaryRepairInfo(object): 
    ...
    def __init__(self,clone = None): 
        self.distance: int=0
        self.misspellIndex: int=0
        self.code: int=0
        self.bufferPosition: int=0
        self.symbol: int=0
...
class SecondaryRepairInfo(object): 
    def __init__(self) -> None:
        ...
        self.distance: int=0
        self.bufferPosition: int=0
        self.stackPosition: int=0
        self.numDeletions: int=0
        self.symbol: int=0
        self.recoveryOnNextStack: bool=False
...
class DiagnoseParser(Stacks):   
    ...
    def __init__(self,tokStream: TokenStream, prs: ParseTable, maxErrors: int=0, maxTime: int=0,monitor: Monitor=None): 
        ...
        self.EOLT_SYMBOL: int=0
        self.ACCEPT_ACTION: int=0
        self.ERROR_ACTION: int = 0

        self.sym_list : list = []

        self.maxErrors: int = 0

        self.maxTime: int = 0


        self.stateStackTop: int = -1
        self.stateStack: list =  []

        self.locationStack: list =  []

        self.tempStackTop: int = -1
        self.tempStack: list =  []

        self.prevStackTop: int = -1
        self.prevStack: list =  []

        self.nextStackTop: int = -1
        self.nextStack: list =  []

        self.scopeStackTop: int = -1
    ...
    def diagnoseEntry1(self,marker_kind: int) -> int: 
        ...
        #
        error_token: int = self.parseForError(current_kind)
        if (error_token != 0): 
    ...
    def diagnoseEntry2(self,marker_kind: int, error_token: int):
        ...
        while act != self.ACCEPT_ACTION :
            ...
            if (act == self.ERROR_ACTION): 
                ...
                candidate: RepairCandidate = self.errorRecovery(current_token)
                #
                # if the parser needs to stop processing,
                # it may do so here.
                #
                ...
                if (candidate.symbol == 0): 
                    ...
                    if (candidate.symbol > self.NT_OFFSET): 
                        lhs_symbol: int = candidate.symbol - self.NT_OFFSET
                        act = self.ntAction(act, lhs_symbol)
                        while (act <= self.NUM_RULES): 
                ...
                #
                next_token: int = self.tokStream.peek()
                self.tempStackTop = self.stateStackTop
                arraycopy(self.stateStack, 0, self.tempStack, 0, self.stateStackTop + 1)
    ...
    def parseForError(self,current_kind: int) -> int: 
        ...
        #
        curtok: int = self.tokStream.getPrevious(self.tokStream.peek())
        act: int = self.tAction(self.tempStack[self.tempStackTop], current_kind)
        #
    ...
    def parseCheck(self,stack, stack_top: int, first_symbol: int, buffer_position: int) -> int: 
        ...
        local_stack: list =  [0]*(stack.__len__())
        local_stack_top: int = stack_top
        ...
        #
        act: int = local_stack[local_stack_top]
        if (first_symbol > self.NT_OFFSET): 
            lhs_symbol: int = first_symbol - self.NT_OFFSET
    ...
    #
    def errorRecovery(self,error_token: int) -> RepairCandidate :  
        prevtok: int = self.tokStream.getPrevious(error_token)
        ...
        #
        candidate: RepairCandidate = self.primaryPhase(error_token)
        if (candidate.symbol != 0): 
            return candidate
        ...
        # states.
        #
        scope_repair: PrimaryRepairInfo =  PrimaryRepairInfo()
        scope_repair.bufferPosition = self.BUFF_UBOUND
        top: int = self.stateStackTop
        while top >= 0:  
    ...
    def primaryPhase(self,error_token: int) -> int:  
        ...
        #
        i: int = (3 if self.nextStackTop >= 0 else 2)
        ...
        #
        repair: PrimaryRepairInfo =  PrimaryRepairInfo()
        ...
        #
        base_repair: PrimaryRepairInfo =  PrimaryRepairInfo(repair)
        ...
        if (self.prevStackTop >= 0): 
            prev_repair: PrimaryRepairInfo =  PrimaryRepairInfo(repair)
        ...
        # phase recovery.
        #
        candidate: RepairCandidate =  RepairCandidate()
    ...
    def mergeCandidate(self,state: int, buffer_position: int) -> int: 
        ...
        while ( self.asr(k) != 0): 
            i: int = self.terminalIndex(self.asr(k))
    ...
    def checkPrimaryDistance(self,repair: PrimaryRepairInfo, stck, stack_top: int) -> int: 
        ...
        #
        scope_repair: PrimaryRepairInfo =  PrimaryRepairInfo(repair)
        ...
        #
        symbol: int = self.mergeCandidate(stck[stack_top], repair.bufferPosition)
        if (symbol != 0): 
            j: int = self.parseCheck(stck, stack_top, symbol, repair.bufferPosition + 2)
        ...
        #
        j: int = self.parseCheck(stck,
                                        stack_top,
                                        self.tokStream.getKind(self.buffer[repair.bufferPosition + 1]),
                                        repair.bufferPosition + 2)

        k: int =  (    10 if self.tokStream.getKind(self.buffer[repair.bufferPosition]) == self.EOLT_SYMBOL and
        ...
        symbol = root
        while (symbol != 0): 
            m: int = self.parseCheck(stck, stack_top, symbol, repair.bufferPosition)
            n: int = (10 if symbol == self.EOLT_SYMBOL and self.tokStream.afterEol(self.buffer[repair.bufferPosition])
                        else 0)
        ...
        while (symbol != 0): 
            ...
            m: int = self.parseCheck(stck, stack_top, symbol, repair.bufferPosition + 1)
            n: int = (10 if symbol == self.EOLT_SYMBOL and self.tokStream.afterEol(self.buffer[repair.bufferPosition + 1])
        ...
        #
        nt_index: int = self.nasi(stck[stack_top])
        while ( self.nasr(nt_index) != 0 ): 
            symbol = self.nasr(nt_index) + self.NT_OFFSET
            n: int = self.parseCheck(stck, stack_top, symbol, repair.bufferPosition + 1)
    ...
    #
    def primaryDiagnosis(self,repair: PrimaryRepairInfo) -> RepairCandidate :
        #
        #  Issue diagnostic.
        #
        prevtok: int = self.buffer[repair.bufferPosition - 1]
        ...
        if  (repair.code == ParseErrorCodes.INSERTION_CODE or 
             repair.code == ParseErrorCodes.BEFORE_CODE):
                ...
                name_index: int = (self.getNtermIndex(self.stateStack[self.stateStackTop],
                                                        repair.symbol,
                ...
                tok: int = (prevtok if repair.code == ParseErrorCodes.INSERTION_CODE  else current_token)
    ...
    def getTermIndex(self,stck, stack_top: int, tok: int, buffer_position: int) -> int: 
        ...
        threshold: int = self.tempStackTop
    ...
    def getNtermIndex(self,start: int, sym: int, buffer_position: int) -> int: 
        ...
        act: int = self.ntAction(start, highest_symbol)
    ...
    def misspell(self,sym: int, tok: int) -> int: 
        ...
        #
        s1: str = (self.name(self.terminalIndex(sym))).lower()
        n: int = s1.__len__()
        s1 += '\u0000'
        s2: str = (self.tokStream.getName(tok)).lower()
       
        m: int = ( s2.__len__() if s2.__len__() < self.MAX_NAME_LENGTH else self.MAX_NAME_LENGTH)
        s2 = s2[0, m] + '\u0000'
        ...
        while ((i < n) and (j < m)): 
            ...
            elif (s1[i + 1] == s2[i] and s1[i] == s2[j+1]):
    ...
    def scopeTrial(self,repair: PrimaryRepairInfo, stack, stack_top: int) -> int: 
        ...
        for i in range(0, self.stateStack.__len__()):
            self.stateSeen[i] = DiagnoseParser.NIL
        
        self.statePoolTop = 0
        if (self.statePool == None or self.statePool.__len__() == 0 or self.statePool.__len__() < self.stateStack.__len__()): 
            self.statePool = [None]* (self.stateStack.__len__())
    ...
    def scopeTrialCheck(self,repair: PrimaryRepairInfo, stack, stack_top: int, indx: int) -> int: 
        ...
        for i in range(0,self.SCOPE_SIZE): 
            ...
            #
            action.reset()
            act: int = self.tAction(stack[stack_top], self.scopeLa(i))
            ...
            for action_index in range(0, action.size()):
                ...
                if (act != self.ERROR_ACTION): 
                    ...
                    #
                    marked_pos: int = (max_pos + 1 if max_pos < stack_top else stack_top)
                    if (self.scopeRhs(k) == 0 and j < marked_pos): # match?
                        stack_position: int = j
    ...
    #
    def secondaryCheck(self,stack, stack_top: int, buffer_position: int, distance: int) -> bool:
        top: int = stack_top - 1
        while ( top >= 0): 
            j: int = self.parseCheck(stack,
                                            top,
    ...
    #
    def secondaryPhase(self,error_token: int) -> RepairCandidate : 
        repair: SecondaryRepairInfo =  SecondaryRepairInfo()
        misplaced_repair: SecondaryRepairInfo =  SecondaryRepairInfo()

        #
        # If the next_stack is available, try misplaced and secondary
        # recovery on it first.
        #
        next_last_index: int = 0
        if (self.nextStackTop >= 0): 
        ...
        if (repair.code == ParseErrorCodes.SECONDARY_CODE or repair.code == ParseErrorCodes.DELETION_CODE): 
            ...
            while (
                ...
                j: int = (last_index if scope_repair.distance == self.MAX_DISTANCE
                                   else scope_repair.distance)

                k: int = scope_repair.bufferPosition - 1
                if ((scope_repair.distance - k) > self.MIN_DISTANCE and (j - k) > (repair.distance - repair.numDeletions)): 
                    i: int = self.scopeIndex[self.scopeStackTop]# upper bound
                    repair.code = ParseErrorCodes.SCOPE_CODE
    ...
    #
    def misplacementRecovery(self,repair: SecondaryRepairInfo, stack, stack_top: int, last_index: int, stack_flag: bool): 
        ...
        while ( top >= 0 ): 
            ...
            parse_distance: int = self.parseCheck(stack, top, self.tokStream.getKind(self.buffer[2]), 3)
            j: int = (last_index if parse_distance == self.MAX_DISTANCE else parse_distance)
            if ((parse_distance > self.MIN_DISTANCE) and (j - stack_deletions) > (repair.distance - repair.numDeletions)): 
    ...
    #
    def secondaryRecovery(self,repair: SecondaryRepairInfo, stack, stack_top: int, last_index: int, stack_flag: bool): 
        previous_loc: int = self.buffer[2],
        stack_deletions: int = 0
        top: int = stack_top 
        while (top >= 0 and repair.numDeletions >= stack_deletions): 
            ...
            while ( i <= (last_index - self.MIN_DISTANCE + 1) and
                ...
                parse_distance: int = self.parseCheck(stack, top, self.tokStream.getKind(self.buffer[i]), i + 1)
                j: int = (last_index if parse_distance == self.MAX_DISTANCE else parse_distance)

                if ((parse_distance - i + 1) > self.MIN_DISTANCE): 

                    k: int = stack_deletions + i - 1
                    if ((k < repair.numDeletions) or
                        (j - k) > (repair.distance - repair.numDeletions) or
                        ((repair.code == ParseErrorCodes.SECONDARY_CODE) and (j - k) == (repair.distance - repair.numDeletions))):
                ...
                l: int = self.nasi(stack[top])
                while ( l >= 0 and self.nasr(l) != 0 ): 
                    symbol: int = self.nasr(l) + self.NT_OFFSET
    ...
    #
    def secondaryDiagnosis(self,repair: SecondaryRepairInfo): 
    ...
    def emitError(self,msg_code: int, name_index: int, left_token: int, right_token: int, scope_name_index: int=0): 

        left_token_loc: int = (right_token if left_token > right_token else left_token)
        ...
        if (msg_code == ParseErrorCodes.SCOPE_CODE): 
            token_name = "\""
            i: int = self.scopeSuffix(-name_index)
