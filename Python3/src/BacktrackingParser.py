...
class BacktrackingParser(Stacks) :
    ...
    def getFirstToken(self,i: int = None): 
        if (not i): 
    ...
    def getLastToken(self,i : int = None): 
        if (not i): 
    ...
    def reallocateOtherStacks(self,start_token_index: int): 
        if (not self.actionStack or self.actionStack.__len__() == 0): 
            ...
            self.locationStack =  [0]*(self.stateStack.__len__())
            self.parseStack =  [None]*(self.stateStack.__len__())
            self.actionStack[0] = 0
            self.locationStack[0] = start_token_index
        else :
            if (self.actionStack.__len__() < self.stateStack.__len__()): 
                old_length: int = self.actionStack.__len__()
                self.actionStack = arraycopy(self.actionStack, 0,  [0]*(self.stateStack.__len__()), 0, old_length)
                self.locationStack = arraycopy(self.locationStack, 0,  [0]*(self.stateStack.__len__()), 0, old_length)
                self.parseStack =arraycopy(self.parseStack, 0,   [None]*(self.stateStack.__len__()), 0, old_length)
    ...
    def fuzzyParseEntry(self,marker_kind: int, max_error_count: int = None) :
        ...
        if (error_token != 0): # an error was detected?
            ...
            rp =  RecoveryParser(self, self.action, self.tokens,self.tokStream, self.prs, max_error_count, 0, self.monitor)
    ...
    def parseEntry(self,marker_kind: int=0, max_error_count: int = 0):
        ...
        arraycopy(self.stateStack, 0, temp_stack, 0, temp_stack.__len__())
        ...
        while (error_token != 0):
            ...
            self.stateStackTop = temp_stack.__len__() - 1
            arraycopy(temp_stack, 0, self.stateStack, 0, temp_stack.__len__())
