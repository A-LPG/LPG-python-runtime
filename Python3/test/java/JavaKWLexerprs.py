...
class JavaKWLexerprs(ParseTable,JavaKWLexersym):
    ERROR_SYMBOL : int = 0
    ...
    def isValidForParser(self) -> bool :  return JavaKWLexersym.isValidForParser
    ...
    def isNullable(self, index: int) ->bool : return  JavaKWLexerprs._isNullable[index] != 0

    _prosthesesIndex : list = [0,
            8,7,6,11,9,10,4,12,13,14,
            16,2,3,5,15,1
        ]
    def prosthesesIndex(self, index: int) ->int : return  JavaKWLexerprs._prosthesesIndex[index]
    ...
    def isKeyword(self, index: int) ->bool : return  JavaKWLexerprs._isKeyword[index] != 0
    ...
    def baseCheck(self, index: int) ->int : return  JavaKWLexerprs._baseCheck[index]
    _rhs : list = _baseCheck
    def rhs(self, index: int)  -> int: return JavaKWLexerprs._rhs[index]
    ...
    def baseAction(self, index: int) ->int : return  JavaKWLexerprs._baseAction[index]
    _lhs : list = _baseAction
    def lhs(self, index: int)  -> int: return JavaKWLexerprs._lhs[index]
    ...
    def termCheck(self, index: int) ->int : return  JavaKWLexerprs._termCheck[index]
    ...
    def termAction(self, index: int) ->int : return  JavaKWLexerprs._termAction[index]
    ...
    def tAction(self, state : int,  sym : int)-> int:
        i = JavaKWLexerprs._baseAction[state]
        k = i + sym
        return JavaKWLexerprs._termAction[  k if JavaKWLexerprs._termCheck[k] == sym else i]
    
    def lookAhead(self, la_state : int , sym: int)-> int:
        k = la_state + sym
        return JavaKWLexerprs._termAction[  k if JavaKWLexerprs._termCheck[k] == sym else  la_state]
