...
class DeterministicParser(Stacks):
    ...
    def __init__(self, tokStream=None, prs=None, ra=None, monitor=None):
        ...
        super(DeterministicParser, self).__init__()
    ...
    #
    def tAction(self, act, sym, index):
    ...
    def reset(self, tokStream=None, prs=None, ra=None,
        ...
        if tokStream is None:
            self.reset1()
            return

        self.reset2(tokStream, monitor)

    def parseEntry(self, marker_kind=0):
        #
        ...
        while True:
            ...
            # it may do so here.
            #
            if self.monitor is not None and self.monitor.isCancelled():
    ...
    # using the entry point parse(int [], int).
    #
    def resetParserEntry(self, marker_kind):
    ...
    #
    def errorReset(self):
        gate = (0 if self.markerKind == 0 else 1)
    ...
    def parse(self, sym, index):
        ...
        # offset here.
        #
        save_action_length = self.action.size()
        pos = self.stateStackTop
        location_top = self.stateStackTop - 1
        ...
        while self.currentAction <= self.NUM_RULES:
            ...
            self.currentAction = self.tAction(self.currentAction, sym, index)  # for loop
    ...
    def parseActions(self):
        ...
        self.tokStream.reset()
        self.lastToken = self.tokStream.getPrevious(self.tokStream.peek())
        curtok = (self.tokStream.getToken() if self.markerKind == 0 else self.lastToken)
